...
| Scope and filter behavior | Expected resolved/contributing subscriptions; non-empty in/out-of-scope evidence for selected subscription, RG, scanner and tag filters; parent-group recursion or explicit accepted limit | Subscription, two-subscription, resource-group CLI, leaf management group, Storage/VM, RG include and tag-only include filters have live comparisons; selected tag IDs match the unfiltered baseline and the RG filtered ID set. The cross-run Advisor count difference is consistent with source-compatible unknown tag-scope exclusion, though Azure timing cannot be ruled out; parent-to-child management-group recursion and remaining include/exclude interactions remain open |
