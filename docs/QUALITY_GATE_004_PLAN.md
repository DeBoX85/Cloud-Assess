...
| Scope and filter behavior | Expected resolved/contributing subscriptions; non-empty in/out-of-scope evidence for selected subscription, RG, scanner and tag filters; parent-group recursion or explicit accepted limit | Subscription, two-subscription, resource-group CLI, leaf management group, Storage/VM, RG include and tag-only include filters have live comparisons; selected tag IDs match the unfiltered baseline. The cross-run Advisor count change needs classification; parent-to-child management-group recursion and remaining include/exclude interactions remain open |
