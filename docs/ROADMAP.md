...
- The `AdvisoryDev` RG include-filter pass returned exact agreement with three in-scope resources and non-empty findings and Advisor data (Phase R). The local target-report check confirmed one expected Dev subscription, `complete` status, and no stage warnings.
- The separate `Environment: dev` tag-only pass returned exact source/target agreement with three selected resources and no target stage warnings (Phase S). The user's local ID-set check matched exactly the three tagged resources in the earlier unfiltered baseline and excluded the nine nonmatching resources. Advisor returned 2/2 rows versus 3/3 in the earlier RG-only pass; directly compare the two filtered runs' selected IDs and classify the row change before attributing it to Azure timing or downstream filter behavior.

The next live work items are classifying the cross-run Advisor difference, nested management-group traversal in a suitable non-production hierarchy, and remaining filter combinations. Diagnostics warning caveats from Phases N and P remain in the evidence matrix. Core equivalence and a release decision require the open validation limits to be resolved or explicitly accepted with a narrowed scope.
...
The permanent CI workflow is a development regression gate. An active ruleset protects `bootstrap/core-v1` with required pull requests and both `quality` and `windows-validation` checks; see the enforcement checkpoint in the ledger. The dependency and rule maintenance workflows publish proposed changes to separate branches and provide compare links for PR creation; they no longer push directly to the protected branch. Their postchange manual-dispatch behavior has not been exercised yet.
...
- Validate the remaining filter interactions in paired source/target schemas: subscription and resource-group include/exclude precedence, other service/scanner selections, individual-resource and recommendation exclusions, exclude tags, and parent/child tag scope. The paired Storage/VM scanner-selection, Dev RG include and separate `Environment: dev` tag include passes matched with reviewed target stage health (Phases Q–S). Classify the 3-to-2 Advisor row change across the latter two historical runs before treating all downstream tag effects as settled. Cover multiple resource groups in one subscription where data permits.
