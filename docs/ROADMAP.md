...
- The separate `Environment: dev` tag-only pass returned exact source/target agreement with three selected resources and no target stage warnings (Phase S). The user's local ID-set check matched exactly the three tagged resources in the earlier unfiltered baseline and excluded the nine nonmatching resources. A later local comparison confirmed the RG-only and tag-only selected ID sets were equal. Advisor returned 2/2 versus 3/3; the one absent row targeted neither a selected resource nor its child. Both pinned source and target record excluded inventory decisions and exclude unknown scope under an include-tag filter. The absent row was not checked against out-of-scope inventory, and Azure timing remains possible, so its exact cross-run cause is open.
