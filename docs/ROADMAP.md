...
- The `AdvisoryDev` RG include-filter pass returned exact comparator-level agreement with three in-scope resources and non-empty findings and Advisor data (Phase R). Target scope and stage health have not yet been supplied for this pass. The separate tag-only fixture remains unrun and requires a baseline selectivity check.
...
- Validate the remaining filter interactions in paired source/target schemas: subscription and resource-group include/exclude precedence, other service/scanner selections, individual-resource and recommendation exclusions, include/exclude tags, and parent/child tag scope. The paired Storage/VM scanner-selection fixtures matched with reviewed stage health (Phase Q). The Dev RG include pair matched semantically (Phase R), with its target stage-health and scope review pending. The separate `Environment: dev` pair remains unrun and needs a check that the unfiltered inventory contains nonmatching resources. Cover multiple resource groups in one subscription where data permits.
