...
- The `AdvisoryDev` Storage/VM scanner-selection pass returned exact comparator-level semantic agreement with non-empty datasets (Phase Q). Its supplied metadata and comparison omit the resolved subscription IDs and target stage health; those checks remain pending before this filtered pass can be considered fully reviewed.
...
- Validate the remaining filter interactions in paired source/target schemas: subscription and resource-group include/exclude precedence, service/scanner selection, individual-resource and recommendation exclusions, include/exclude tags, and parent/child tag scope. The paired Storage/VM scanner-selection fixtures in [the equivalence guide](EQUIVALENCE.md#filter-file-note) matched semantically on the Dev leaf group (Phase Q); review the retained target report's stage health and resolved subscription before closing that pass. Cover multiple resource groups in one subscription where data permits.
